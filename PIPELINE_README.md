...
│ 720p RGB │ COCO+深度│ SAM      │ DA3逐帧  │ TripoSR  │  ★核心改进  │
├──────────┴──────────┴──────────┴──────────┴──────────┴─────────────┤
│                 M4 XMem 时序掩码跟踪 (平滑连贯)                    │
├─────────────────────────────────────────────────────────────────────┤
...
| 项目 | 说明 |
|------|------|
| 模块 | `modules/triposr_mesh_generator.py` |
| 模型 | TripoSR (1.6GB) |
| 输入 | 首帧 RGB + 掩码 |
| 输出 | `.glb` 网格文件（Marching Cubes 128³） |
| 备选 | 失败时退化为 bbox 代理网格 |

> 网格生成采用 **TripoSR**（单视图静态重建）。Stream3D 流式多视图重建可补全背面几何，但目视追踪框朝向误差更大，当前不采用。
...
├── meshes/
│   ├── proxy_mesh.glb         # TripoSR 原始网格
│   └── proxy_mesh_aligned.glb # 尺度对齐后的网格 (mm 单位)
├── debug/                     # 检测/分割/深度可视化
└── K.npy                      # 相机内参
```
