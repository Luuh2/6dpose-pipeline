...
- **★ 静物速度归零**：`motion_level < 0.2` 时 LIEKF 速度项强制置 0，停止积分——杜绝静物时滚转漂移（静止物体框必须不动）
- **失败恢复**：掩码面积骤变 / 追踪 bbox 与掩码 IoU 过低 → 连续 2 次才触发重注册兜底（防单帧抖动）+ 重注册结果 SE(3) 平滑插值
...
| **多帧检测** | ★ 首帧目标不清晰时自动尝试 1/4、1/2、3/4、9/10 处帧，用检出帧做分割/网格/尺度对齐 |
...
### 6D 框每隔约 1 秒抖动/跳变（已修复 ✅）

**原因**：CONFIDENCE_CHECK 里硬编码 360p 裁剪（`min(479,x2), min(359,y2)`），720p 视频追踪 bbox 被错误裁剪 → IoU 计算错误 → 每 30 帧误触发重注册（全局搜索）→ pose 跳变。

**解决**：① 裁剪改用实际帧尺寸 `min(wp-1,x2), min(hp-1,y2)`；② 重注册改为 SE(3) 平滑插值 20%（不直接跳变）；③ 连续 2 次低 IoU 才重注册（防单帧误判）。

### 静物时 6D 框滚转漂移（已修复 ✅）

**原因**：LIEKF 速度模型在静物时累积偏差 → 每帧外推一点 → 平滑滚转漂移。

**解决**：`motion_level < 0.2` 时 LIEKF 速度项强制置 0，停止积分。
