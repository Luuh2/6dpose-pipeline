"""Improved tracking: better K (f=288), better mask (39.6%), varied depth."""
import sys
...
BASE='/mnt/e/zhijiyige'; OUT=f'{BASE}/output_improved'
...
print(f"[Data] depth [{depths.min():.2f},{depths.max():.2f}]m mask={masks[0].sum()}px ({masks[0].sum()/(hp*wp)*100:.1f}%)")

# IMPROVED K: f=288
K=np.loadtxt(f'{OUT}/K_improved.txt')
print(f"[Cam] K (f={K[0,0]:.0f}, improved heuristic):\n{K}")
